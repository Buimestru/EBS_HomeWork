...
public class SubscriptionGenerator {
    ...
    public static List<String> generateAllSubscriptions() {
        int totalSubs = Config.TOTAL_SUBSCRIPTIONS;
        List<List<Subscription>> allSubs = new ArrayList<>(totalSubs);
        ...
        for (Map.Entry<String, Integer> entry : Config.FIELD_FREQUENCIES.entrySet()) {
            ...
            for (int i = 0; i < fieldCount; i++) {
                ...
                allSubs.get(subIndex).add(new Subscription(field, operator, value));
            }
        }
        ...
        for (int emptyIdx : emptyIndexes) {
            ...
            List<Subscription> donorList = allSubs.get(donorIdx);
            Subscription subToMove = donorList.removeFirst();
        ...
        for (int t = 0; t < Config.NUM_THREADS; t++) {
            ...
            futures.add(executor.submit(() -> {
                ...
                for (int i = start; i < end; i++) {
                    List<Subscription> subs = allSubs.get(i);
                    String line = "{" + String.join(";", subs.stream().map(Object::toString).toList()) + "}";
                    localList.add(line);
                }
                return localList;
            }));
        ...
        List<String> result = new ArrayList<>(totalSubs);
    ...
    public static void writeToFile(List<String> subscriptions, String filename) throws Exception {
